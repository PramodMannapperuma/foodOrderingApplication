...
class DataService {
  Future<Map<String, dynamic>> loadJsonData() async {
    try {
      String jsonString = await rootBundle.loadString(
          'assets/assignment-dataset.json'); // Update the path as per your JSON file
      Map<String, dynamic> jsonData = json.decode(jsonString);
      debugPrint("JSON Loaded: $jsonData"); // Log the data
      return jsonData;
    } catch (e) {
  ...
  Future<List<Item>> fetchMenuItems(String categoryID) async {
    try {
      // Load JSON data (this should be replaced with actual API request logic)
      final data = await loadJsonData();
      final itemsData = data['Result']['Items']; // Access Items under Result
